import { Habit, HabitPackage, HabitFrequency } from '@/types';
...
export const aiRoutineArchitect = {
  async generatePackage(request: AIRequest): Promise<AIResponse> {
    ...
    let response: Omit<AIResponse, 'habits'> & { habits: any[] };
    ...
    } else if (prompt.includes('typescript') || prompt.includes('coding') || prompt.includes('developer')) {
      response = {
